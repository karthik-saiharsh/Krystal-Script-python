...
IF_PATTERN = "^if"
IF_ENDS_PATTERN = "IFEND"
ELSE_PATTERN = "Else do"
REPEAT_ENDS_PATTERN = "REPEND"
...
for line in code:
    ...
    elif line == IF_ENDS_PATTERN:
        if args[2] == "linux":
            write_code("fi")
        else:
            write_code("}")

    elif line == ELSE_PATTERN:
        if args[2] == "linux":
            write_code("else")
        else:
            write_code("} else {")

    elif line == REPEAT_ENDS_PATTERN:
        if args[2] == "linux":
            write_code("i=$((i+1))\ndone")
        else:
            write_code("}")

    elif line == WAIT_PATTERN:
        if args[2] == "linux":
            write_code("read _")
        else:
            write_code('Read-Host -Prompt ""')
    ###### TOGGLE CONDITIONS ######
    ...
    ########## MAIN CODE ##########
        ...
        if (gen_code := stdlibks.Display_fn(line, args[2])) != "err":
            ...
            FOUND_ERROR = True
            errors.append(code.index(line)+1)
    ...
    elif re.search(IF_PATTERN, line.lower()):
        if (gen_code := stdlibks.If(line, args[2])) != "err":
            write_code(gen_code)
        else:
            FOUND_ERROR = True
            errors.append(code.index(line)+1)

    elif re.search(REPEAT_PATTERN, line.lower()):
        if (gen_code := stdlibks.Repeat(line, args[2])) != "err":
            write_code(gen_code)
        else:
            FOUND_ERROR = True
            errors.append(code.index(line)+1)
...
if FOUND_ERROR:
    ...
    print(f"\033[0;31mErrors were found on the following lines:\033[0;0m")
    for j in errors:
        print(f"\033[0;31m{j}\033[0;0m")  
    os.remove(filename)